...
export function CurriculumStructure({
  ...
  const discardStale = useDiscardStaleDrafts(grade, subject);
  ...
  const serverDrafts = React.useMemo(() => {
    const map: Record<
      string,
      { jobId: string; subs: GeneratedSubstrand[]; stale: boolean; missing: string }
    > = {};
  ...
  const staleDrafts = React.useMemo(
    () => (queuedDrafts.data?.drafts || []).filter((d) => d.stale),
    [queuedDrafts.data]
  );
  ...
  return (
    ...
    >
      ...
      {staleDrafts.length > 0 && (
        ...
        >
          <strong>
            {staleDrafts.length} draft(s) were produced by an older generator
          </strong>
          <div style={{ color: "var(--ink-3)", marginTop: 4 }}>
            A queued draft outlives the code that made it, so these look exactly
            like fresh output and are not. What they are missing:{" "}
            {staleDrafts[0].missing}
          </div>
          <ul style={{ margin: "var(--s2) 0", paddingLeft: "1.2em" }}>
            {staleDrafts.map((d) => (
              <li key={d.job_id}>
                <code>{d.strand_name}</code>
              </li>
            ))}
          </ul>
          <Button
            size="sm"
            variant="danger"
            disabled={discardStale.isPending}
            loading={discardStale.isPending}
            title="Throw these away so the strands can be generated again with the current generator"
            onClick={() => discardStale.mutateAsync()}
          >
            Discard {staleDrafts.length} stale draft(s)
          </Button>
        </div>
      )}
      {outstanding > 0 && (
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "var(--s3)" }}>
                  ...
                  {isDraft && (
                    <Badge tone={queuedDraft?.stale ? "danger" : "warn"}>
                      {draftCount} draft
                      {queuedDraft?.stale
                        ? " (older generator)"
                        : queuedDraft && !draft
