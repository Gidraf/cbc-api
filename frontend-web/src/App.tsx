...
export function App() {
  ...
  const [stageDrafts, setStageDrafts] = useState<Record<Stage, { provider: Provider; model: string; base_url: string }>>({
    notes_generation: { provider: "openai", model: "gpt-4o", base_url: "" },
    diagram_generation: { provider: "openai", model: "gpt-4o", base_url: "" },
    activity_generation: { provider: "openai", model: "gpt-4o", base_url: "" },
    question_generation: { provider: "openai", model: "gpt-4o", base_url: "" },
    reviewer_panel: { provider: "openai", model: "gpt-4o", base_url: "" },
    regeneration: { provider: "openai", model: "gpt-4o", base_url: "" }
  });
            ...
            <div className="panel-head">
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  onClick={async () => {
                    await run("Bootstrap OpenAI Bindings", async () => {
                      const res = await fetchJson<any>("/admin/pipeline-bindings/bootstrap", {
                        method: "POST",
                        body: JSON.stringify({ provider: "openai", model: "gpt-4o", base_url: null }),
                      }, auth());
                      setStageDrafts({
                        notes_generation: { provider: "openai", model: "gpt-4o", base_url: "" },
                        diagram_generation: { provider: "openai", model: "gpt-4o", base_url: "" },
                  ...
                  disabled={isRunning}
                >
                  ⚡ Set All Stages to OpenAI (gpt-4o)
                </button>
              </div>
            </div>
                      ...
                      <label style={{ fontSize: "12px" }}>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const defaultModel = p === "openai" ? "gpt-4o" : (p === "anthropic" ? "claude-3-5-sonnet-20241022" : (p === "gemini" ? "gemini-2.0-flash" : "llama3.1"));
                            setStageDrafts({
                              ...stageDrafts,
