...
export function App() {
  ...
  const [stageDrafts, setStageDrafts] = useState<Record<Stage, { provider: Provider; model: string; base_url: string }>>({
    notes_generation: { provider: "openai", model: "gpt-5.6-terra", base_url: "" },
    diagram_generation: { provider: "openai", model: "gpt-5.6-terra", base_url: "" },
    activity_generation: { provider: "openai", model: "gpt-5.6-terra", base_url: "" },
    question_generation: { provider: "openai", model: "gpt-5.6-terra", base_url: "" },
    reviewer_panel: { provider: "openai", model: "gpt-5.6-terra", base_url: "" },
    regeneration: { provider: "openai", model: "gpt-5.6-terra", base_url: "" }
  });
                ...
                <button
                  onClick={async () => {
                    await run("Bootstrap OpenAI Bindings", async () => {
                      ...
                      // default (OPENAI_DEFAULT_MODEL for stages that write,
                      // OPENAI_LIGHT_MODEL for stages that read).
                      const res = await fetchJson<any>("/admin/pipeline-bindings/bootstrap", {
                        method: "POST",
                        body: JSON.stringify({ provider: "openai", model: "", base_url: null }),
                      }, auth());
                      const bound: Record<string, string> = res?.bindings || {};
                      const draft = (stage: string) => ({ provider: "openai" as const, model: bound[stage] || "", base_url: "" });
                      setStageDrafts({
                        notes_generation: draft("notes_generation"),
                        diagram_generation: draft("diagram_generation"),
                        activity_generation: draft("activity_generation"),
                        question_generation: draft("question_generation"),
                        reviewer_panel: draft("reviewer_panel"),
                        regeneration: draft("regeneration"),
                      });
                      return res;
                    });
                  }}
                  disabled={isRunning}
                >
                  ⚡ Set All Stages to OpenAI defaults (by tier)
                </button>
              </div>
            </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: "8px", alignItems: "flex-end" }}>
                      <label style={{ fontSize: "12px" }}>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const defaultModel = p === "openai" ? "gpt-5.6-terra" : (p === "anthropic" ? "claude-3-5-sonnet-20241022" : (p === "gemini" ? "gemini-2.0-flash" : "llama3.1"));
                            setStageDrafts({
                              ...stageDrafts,
                      ...
                      <label style={{ fontSize: "12px" }}>
                        ...
                        <input
                          ...
                          placeholder="e.g. gpt-5.6-terra, gpt-5.6-luna, gpt-5.6-sol, claude-3-5-sonnet-20241022"
                          style={{ marginTop: "4px" }}
                        />
                      </label>
                      ...
                      <button
                        onClick={() => run(`Save ${stage}`, () => fetchJson(`/admin/pipeline-bindings/${stage}`, {
                          ...
                          body: JSON.stringify({
                            ...
                            model: draft.model || "gpt-5.6-terra",
